...
def write_to_csv(key, landmark_list):
    if 48 <= key <= 57: # 0 ~ 9
        csv_path = './keypoint.csv'
        key = key - 48
        with open(csv_path, 'a', newline='') as f:
            writer = csv.writer(f)
            writer.writerow([key, *landmark_list])
        cv.putText(image, str(key), (10, 100), cv.FONT_HERSHEY_SIMPLEX, 2.0, (0, 255, 255), 3, cv.LINE_AA)
...
while True:
    ...
    if results.multi_hand_landmarks:
        for hand_landmarks in results.multi_hand_landmarks:
            ...
            write_to_csv(key, pre_processed_landmark_list)
